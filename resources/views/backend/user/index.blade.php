		...
		<table  class="ui celled table" cellspacing="0" width="100%">
		 	<thead>
		        <tr class="text-center">
		            ...
		            <th class="sortable" data-sort="user-table-verified">Verified <i class="sort icon"></i></th>
		    ...
		    @if(count($users))
		    	@foreach($users as $user)
			    	<tr>
			    		...
			    		<td class="user-table-verified">{{ $user->email_verified_at ? 'verified' : 'not verified' }}</td>
			    		<td class="user-table-display-name">{{$user->display_name}}</td>
			    		<td class="user-table-created-at">{{$user->created_at->format('d M Y')}}</td>
			    		<td>
			    			<a href="{{url('dashboard/users/'.$user->id.'/edit')}}" id="edit-user-{{$user->id}}"  class="mini ui button positive"><i class="edit icon"></i> Edit</a>
